...
/* sometimes the abbreviation for door,dr, is spelled uppercase in model and somtimes 
		lowercase.. we want lowercase */

model = tranwrd(model,'4DR','4dr');
model = tranwrd(model,'2DR','2dr');

/* DRloc = find(model,'DR'); returns the location of DR in model via a string count */
...
proc print data=carsCompare(obs=10);
run;

proc print data=carsBase(obs=10);
run;

/* removing a leading space */
data CarsModified;
	set sashelp.cars; /* can reasd a SaS table (or other structured tables in a library) using SET */
model = left(model);
run;
...
proc compare base=CarsModified
	compare=CarsCompare(rename=(CityMPG = MPG_City HighwayMPG = MPG_Highway))
	out = differences outall outnoequal;
run;
...
where type not in ('Sport');
run;

proc sgplot data=sashelp.class pctlevel=graph;
  vbar age / response=height stat=percent
     group=sex;
run;
